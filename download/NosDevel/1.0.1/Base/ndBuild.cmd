...
IF !ndExit! NEQ 0 GOTO :Install.Check-Error
CALL :Install.CheckFile "!ndBuildDir!Lib\REPL.BAT"
IF !ndExit! NEQ 0 GOTO :Install.Check-Error
...
CALL :Tool.Verify Where
CALL :App.Write "Searching for tool: %~1" 7 Tool.FindInPath
"!ndToolWhere!" "%~2" > "!ndTempDir!%~1.output" 2>&1
IF %ErrorLevel% EQU 0 (
...
FOR /F "tokens=1 delims=," %%i in ('TYPE "!ndTempDir!Update-FileList.csv"') DO (
    ...
    ECHO %%i| !ndBuildDir!Lib\REPL.BAT "\\" "/"> "!ndTempDir!DownloadFile.tmp"
